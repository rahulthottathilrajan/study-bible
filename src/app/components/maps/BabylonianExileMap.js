...
export const BABYLONIAN_DETAILS = {
  ...
  babylon:   { title:"Babylon — The Great City",             body:"Babylon under Nebuchadnezzar was the greatest city on earth — ancient sources claimed its walls stretched 56 miles around (archaeological evidence suggests the inner circuit was ~11 miles). The legendary Hanging Gardens were one of the seven wonders. The exiled Jews settled here for 70 years. Daniel served in Nebuchadnezzar's court. The Jews by the rivers of Babylon wept and hung their harps on the willows.", scripture:"Psalm 137:1 — 'By the rivers of Babylon, there we sat down, yea, we wept, when we remembered Zion.'" },
...
export default function BabylonianExileMap({ onSelectLocation, selectedLocation, dark }) {
  return (
    <MapCard dark={dark} legend={<>
      <LegendDot dark={dark} color="#E8625C" label="City Falls" />
      <LegendDot dark={dark} color="#D4A853" label="Babylon" />
      <LegendDot dark={dark} color="#8B5CF6" label="Exile Settlement" />
      <LegendDot dark={dark} color="#1B7A6E" label="Prophet" />
      <LegendDot dark={dark} color="#C06C3E" label="Exile Route" />
    </>}>
      <svg viewBox="0 0 110 80" style={{ width:"100%", display:"block" }} role="img" aria-label="Map of the Babylonian exile">
        <SeaDef id="sea-bab" />
        <ParchBg w={110} h={80} vigId="vig-bab" />

        {/* Mediterranean */}
        <polygon points="0,0 20,0 22,14 18,36 14,52 0,52 0,0" fill="url(#sea-bab)" opacity="0.78" className="sea-shimmer" />
        ...
        {/* Red Sea */}
        <polygon points="16,58 26,54 30,66 24,76 14,74 12,64" fill="url(#sea-bab)" opacity="0.60" className="sea-shimmer" />

        {/* Persian Gulf */}
        <polygon points="80,62 96,58 100,72 84,76 76,68" fill="url(#sea-bab)" opacity="0.65" className="sea-shimmer" />
        ...
        {/* Euphrates */}
        <path d="M60,14 C60,24 58,36 56,48 C54,58 52,66 50,76" fill="none" stroke="#7AAEC8" strokeWidth="1.6" opacity="0.60" strokeLinecap="round" className="river-shimmer" />
        <HaloText x={52} y={44} text="Euphrates" fontSize={2.2} opacity={0.55} color={P.inkFaint} anchor="end" />

        {/* Tigris */}
        <path d="M74,12 C74,22 72,34 70,46 C68,56 66,64 64,74" fill="none" stroke="#7AAEC8" strokeWidth="1.4" opacity="0.55" strokeLinecap="round" className="river-shimmer" />
        <HaloText x={76} y={42} text="Tigris" fontSize={2.2} opacity={0.55} color={P.inkFaint} anchor="start" />

        {/* Jordan River */}
        <path d="M38,26 C38,34 38,44 38,54 C36,62 34,68 32,74" fill="none" stroke="#7AAEC8" strokeWidth="1.2" opacity="0.55" strokeLinecap="round" className="river-shimmer" />

        {/* Dead Sea */}
        <ellipse cx="37" cy="54" rx="2.2" ry="4.5" fill="#7EB8D4" opacity="0.72" className="sea-shimmer" />
