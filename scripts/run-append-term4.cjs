// run-append-term4.cjs
// Combines Term 4 batches A+B and appends to TeensCurriculumData.js
const fs = require("fs");
const path = require("path");

const TARGET = path.join(__dirname, "../src/app/components/TeensCurriculumData.js");

const BATCH_A = require("./append-term4-teens-a.cjs");
const BATCH_B = require("./append-term4-teens-b.cjs");

const ALL_TERM4 = [...BATCH_A, ...BATCH_B];

console.log(`Loaded ${ALL_TERM4.length} Term 4 lessons (weeks ${ALL_TERM4[0].week}–${ALL_TERM4[ALL_TERM4.length-1].week})`);
...
// Serializer (same as run-append-term2.cjs and run-append-term3.cjs)
...
let term4Block = "\n  // ═══════════════════════════════════════════════════\n";
term4Block += "  // TERM 4: LIVING FOR GOD (Weeks 40-52)\n";
term4Block += "  // ═══════════════════════════════════════════════════\n\n";

for (const lesson of ALL_TERM4) {
  term4Block += serializeLesson(lesson);
}

const newContent = content.replace(END_MARKER, term4Block + END_MARKER);
...
console.log(`   Added ${ALL_TERM4.length} lessons (W40–W52)`);
